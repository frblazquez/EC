...
char quierePistaBuf[M];	//Buffer para ver si el usuario quiere una pista
int intentos;
	...
	switch (gstate) {
		...
		case ERROR:
			...
			if(intentos > 3)
			{
				// Mensaje de looser
				// Volve a INIT
				uart_send_str(UART0, "Has perdido :(");
				gstate = INIT;
			}
			else
			{
				uart_send_str(UART0, "¿Quieres una pista? [s/n]: ");
				readline(quierePistaBuf, M);

				if(quierePistaBuf[0] == 's')
				{
					if(passwd[0] == guess[0])
						uart_send_str(UART0, "El caracter en la posicion 1 es correcto\n");
					if(passwd[1] == guess[1])
						uart_send_str(UART0, "El caracter en la posicion 2 es correcto\n");
					if(passwd[2] == guess[2])
						uart_send_str(UART0, "El caracter en la posicion 3 es correcto\n");
					if(passwd[3] == guess[3])
						uart_send_str(UART0, "El caracter en la posicion 4 es correcto\n");
				}
				else
				{
					uart_send_str(UART0, "Te quedas sin pista!\n");
				}
